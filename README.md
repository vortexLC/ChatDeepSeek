...
- 适配多家图片生成服务商（基于 OpenAI 兼容协议），自动兼容各服务商的尺寸格式差异
...
- 上下文容量按模型配置（设置 → 服务商，千 token，默认 128K），自动估算已用（消息 + 思考链 + 工具调用 + 搜索结果 + 附件 + 请求固定开销；文档按实际发送的截断上限 20000 字符估算，避免大文件撑爆）
- **自动压缩**：用量达到 **60%** 时调用模型将早期对话浓缩为摘要（保留窗口按 token 预算从最新往回划定，而非固定条数），之后仅发送摘要 + 近期消息，长对话可持续使用；用户点击停止时跳过压缩
- **超容量兜底**：压缩失败或单条消息超大时，自动丢弃最旧消息组以保证请求不超模型上限（工具调用与其结果成组保留，不会拆散导致接口报错）
