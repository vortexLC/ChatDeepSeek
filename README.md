...
| 模型输出的 JSON | 识别为 |
| --- | --- |
| 含 `prompt`，且含 `mode`/`image`/`images` 或当前为 Video 模式 | `generate_video` |
| 含 `prompt`（Image / Agent 模式） | `generate_image` |
| 含 `command`（Build / Agent） | `bash` |
...
- **图片生成**：模型调用 `generate_image` 工具（OpenAI 兼容 `images/generations` 接口），支持尺寸、负面提示词，结果自动下载保存到会话 `images/` 目录并实时推送卡片；阿里云百炼（DashScope）自动把尺寸参数转为其要求的 `1024*1024` 星号格式
- **视频生成后台化**：Video/Agent 模式提交视频生成后立即返回，**后台轮询状态**（硅基流动最长 10 分钟 / 阿里云百炼最长 15 分钟），期间会话可继续正常对话
  ...
  - **先生图再生视频**：`image` 参数支持本会话内已生成/已保存的图片路径（如 `images/xxx.png`，即 `generate_image` 产物），自动读取转 base64 衔接
...
- **产物卡片**：图片显示缩略图、视频显示**首帧截取**（canvas 截帧，asset 协议匿名 CORS 加载）+ 播放角标、文件显示名称与大小，点击可在右侧面板预览（图片查看 / 视频播放 / 文件内容）
...
└── src-tauri/                # 后端 (Rust)
    ...
    └── src/
        ...
        │   ├── anthropic.rs  # Anthropic Messages 协议客户端 (流式 + 思考模式 + 超时/停滞保护 + 摘要)
...
- **模型只回复 JSON 提示词（如 `{"prompt": "..."}`）不生成？** 说明该对话模型不支持 function calling——应用会自动识别文本 JSON 形式的工具参数并**代为执行**生成（Image/Video/Agent/Build 模式均支持）；支持工具调用的模型则走正常调用路径
